...
/**
 * Soft deletes a model instance from the database.
 *
 ...
 *
 * @example
 * const result = await softDelete(1, Article, [{ model: Comment }, { model: Tag }]);
 * if (result) {
...
const softDelete = async (id, model, relationModels) => {
  ...
  const hasRelatedData = relationModels.some(
    (relationModel) => modelData[relationModel.name]?.length > 0,
  );
